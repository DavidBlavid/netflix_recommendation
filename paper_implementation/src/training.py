...
def train_step(df_subset, model, optim, n_movies, device, loss_fn, noising) -> float:
    ...
    pred = model(batch.input_vector) if not noising else model(batch.input_vector+torch.randn_like(batch.input_vector).round().float().to(device))
    ...
    optim.zero_grad()
    batch = df_subset_to_batch(df_subset, n_movies, device, is_test=False)
    pred = model(pred) if not noising else model(pred+torch.randn_like(batch.input_vector).round().float().to(device))
